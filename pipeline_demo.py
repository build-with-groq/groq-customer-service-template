# pipeline_demo.py - Groq Customer Service Pipeline
import time
import asyncio
import logging
...
class GroqCustomerServiceDemo:
    ...
    def start_web_interface(self):
        """Start the web interface for human review"""
        logger.info("Starting web interface...")
        self.human_loop.start_server(port=config.web_ui_port)
        logger.info(f"Web interface started at http://localhost:{config.web_ui_port}")
    ...
    async def process_single_scenario(
        ...
        try:
            ...
            if config.human_review_timeout > 0:
                logger.debug("Phase 3: Human review")
                human_start = time.perf_counter()
                
                try:
                    # Request human review with timeout
                    human_result = await asyncio.wait_for(
                        self.human_loop.request_review(customer_input, ai_response),
                        timeout=config.human_review_timeout
                    )
    ...
    async def process_single_scenario_with_tracking(
        ...
        customer_input: str, 
        scenario_id: str,
        tracker,  # HumanLoopManager instance for tracking
        context: Dict[str, Any] = None
    ) -> Optional[Dict[str, Any]]:
    ...
    async def health_check(self) -> Dict[str, bool]:
        ...
        checks = {
            ...
            "web_interface": self.human_loop.is_running() if hasattr(self.human_loop, 'is_running') else True
        }
    ...
    def update_configuration(
        ...
        if company_name or domain:
            self.response_agent.update_prompt(self.company_name, self.domain)
            self.tone_agent.update_standards(self.company_name, self.domain)
            self.rewrite_agent.update_prompt(self.company_name, self.domain)
            
            logger.info(f"Configuration updated: {self.company_name} {self.domain}")

# Legacy class name for backward compatibility
ExampleHumanLoopDemo = GroqCustomerServiceDemo